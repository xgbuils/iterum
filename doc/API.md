...
- The first `n` values of iterable
...
In addition,
- `[a]` represents a list of values of type `a`.
- `@[a]` represents a list of values of type `a` and it is an instance of `Iterum` class.
- `@(a, b)` represents a list of 2 values that is an instance of `Iterum` class and whose types are `a` and `b`.
...
It builds a new Iterum instance that behaves as the same way as iterable `[a]` parameter and is decorated with all of methods that provides Iterum prototype class.
...
Given an iterable and a number `n`, it returns a new iterable that  excludes the first `n` values of the given iterable.
...
Given a predicate and `iterable`, it returns a new iterable that excludes first values for which the predicate returns truthy.
...
Given iterable, it returns a new iterable of iterable entries. An entry is an iterable pair `(index  value)` where `value` is the value that generates the given iterable and `index` is the position of `value` on the iterable.
...
Given an iterable, it returns a new iterable that generates the values of the given iterable for which `predicate` returns truthy.
...
Given an iterable, a value `val` and a number `n`, it returns the position of first value that is equal to `val` in iterable iteration starting to search from `n`-th value. If iterable does not iterates over this value, it returns `-1`. Values are compared using the [sameValueZero](http://ecma-international.org/ecma-262/7.0/#sec-samevaluezero) specification.
...
Given an iterable and a function, it return an equivalent iterable. function it's only useful for debug and side effects.
...
Given an iterable of iterables, it returns a new iterable of iterables that generates values transposed. It is a generalization of [zip](#zip) for a countable number of iterables.
