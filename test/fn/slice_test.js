...
describe('slice', function () {
    ...
    describe('function', function () {
        it('normal behaviour', function () {
            const sliceIterable = Iterum.slice(1, 8, [5, 7, 10])
            expect([...sliceIterable]).to.be.deep.equal([7, 10])
        })

        it('throws an error if first parameter is not a number', function () {
            function test () {
                Iterum.slice(false, 8, [5, 7, 10])
            }
            expect(test).to.throw(TypeError,
                /^false is not a number$/)
        })

        it('throws an error if second parameter is not a number', function () {
            function test () {
                Iterum.slice(1, false, [5, 7, 10])
            }
            expect(test).to.throw(TypeError,
                /^false is not a number$/)
        })
